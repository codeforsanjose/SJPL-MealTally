...
const addLatestLaundryFormattingList = (sheet) => {
    sheet.getColumn(1).eachCell((cell, index) => {
        if (index > 10) {
            cell.font = {
                size: 16
            }
    ...
    sheet.getColumn(7).eachCell((cell, index) => {
        if (index > 10) {
            cell.font = {
                size: 16
            }
...
function createExcelReport(data, signature) {
    ...
    return new Promise((resolve, reject) => {
        if (data) {
            ...
            addDataToSheet(workbook, sheetChildren, data, 'youth')
            addBlankRow(sheetChildren)
            addBlankRow(sheetChildren)
            
            formatEachCell(sheetChildren)
            addLatestLaundryFormattingList(sheetChildren)
            addFooterRow(workbook, sheetChildren, youthTotals.youthUnderTotal, signature)
            /**-------------------------------------------------------------------------------------- */
            ...
            addDataToSheet(workbook, sheetAdult, data, 'adult')
            addBlankRow(sheetAdult)
            addBlankRow(sheetAdult)

            
            formatEachCell(sheetAdult)
            addLatestLaundryFormattingList(sheetAdult)
            addFooterRow(workbook, sheetAdult, adultTotals.adultTotal, signature)
