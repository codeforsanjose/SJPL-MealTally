...
Handlebars.registerHelper('table', function(items, options) {
    ...
    console.log('pdf creator handlers registerHelper items: ', items)
    console.log('pdf creator handlers registerHelper options: ', options)
    ...
    return new Promise((resolve, reject) => {
        ...
        if (resultHTML && data) { // if we have both data and a resulting html from executing the template
            ...
            pdf.create(resultHTML, { format: 'Letter',"orientation": "landscape" })
                // put the report in the reports directory with the following title
                .toFile("./reports/report-" + new Date().toTimeString() + ".pdf", function(error, result){
